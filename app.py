...
if user_menu == 'Overall Analysis':
    ...
    ## nations
    result = helper.data_over_time(df , 'region')
    fig = px.line(result, x="Year", y="count")
    st.title("Participatint nations over years")
    st.plotly_chart(fig, use_container_width=True)


    ## events
    result = helper.data_over_time(df , 'Event')
    fig = px.line(result, x="Year", y="count")
    st.title("Events over years")
    st.plotly_chart(fig, use_container_width=True)

    ## athlete
    result = helper.data_over_time(df , 'Name')
    fig = px.line(result, x="Year", y="count")
    ...
    dia = x.pivot_table(index = 'Sport' , columns = 'Year' , values='Event' ,aggfunc='count').fillna(0).astype(int)
    ax = sns.heatmap(dia , annot=True , cmap = 'viridis')
    st.pyplot(fig)
